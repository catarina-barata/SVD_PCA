...
def compute_svd(X_data):
    ...
    # Full Economy SVD: X_c = U * Delta * Vt
...
- **PCA Scores via SVD:** $\mathbf{Z} = \mathbf{U}_r \mathbf{\Delta}_r$
- **Variance relation:** $\lambda_i = \frac{\sigma_i^2}{N - 1}$
""")
...
# -----------------------------------------------------------------------------
# Section 2: Projections Equivalence: X_c * V_r vs. U_r * Delta_r
# -----------------------------------------------------------------------------
st.markdown("---")
...
2. **Via Left Singular Vectors and Singular Values ($U_r, \Delta_r$):** $\mathbf{Z}_{\text{samples}} = \mathbf{U}_r \mathbf{\Delta}_r$
""")
...
Z_from_U = np.dot(U_r, Delta_r)          # Projection via Samples:  (N x r) * (r x r) -> N x r

# Calculate absolute numerical difference between both projections
max_diff = np.max(np.abs(Z_from_V - Z_from_U))

st.success(f"**Maximum Absolute Difference between $(\\mathbf{{X}}_c \\mathbf{{V}}_r)$ and $(\\mathbf{{U}}_r \\mathbf{{\\Delta}}_r)$:** `{max_diff:.10e}` (Numerically Identical)")

col_proj1, col_proj2 = st.columns(2)

with col_proj1:
    st.subheader(r"Projection using $\mathbf{X}_c \mathbf{V}_r$")
    if r >= 2:
...
with col_proj2:
    st.subheader(r"Projection using $\mathbf{U}_r \mathbf{\Delta}_r$")
    if r >= 2:
...
# -----------------------------------------------------------------------------
st.markdown("---")
st.header(r"3. Image Reconstruction using $r$ Components")

# Reconstruct low-rank approximation: X_hat = Mean + U_r * Delta_r * V_r^T
